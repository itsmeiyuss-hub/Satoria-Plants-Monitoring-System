...
async function initDB() {
  ...
  // Insert default plants HANYA jika tabel benar-benar kosong
  const { rows: ep } = await pool.query('SELECT COUNT(*) as count FROM plants');
  if (parseInt(ep[0].count) === 0) {
    const defaultPlants = [
  ...
  const { rows: es } = await pool.query('SELECT COUNT(*) as count FROM spots');
  if (parseInt(es[0].count) === 0) {
    const defaultSpots = [
  ...
  const { rows: eb } = await pool.query('SELECT COUNT(*) as count FROM buildings');
  if (parseInt(eb[0].count) === 0) {
...
// Middleware: init DB sekali saja
...
app.put('/api/plants/:id', async (req, res) => {
  try {
    ...
    // Fallback ke nilai lama jika field tidak dikirim atau kosong
    ...
    // Foto: gunakan nilai baru HANYA jika ada dan tidak kosong, otherwise pakai lama
